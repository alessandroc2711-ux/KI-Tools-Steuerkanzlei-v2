...
Mit vollständlicher Selbstbedienungs-Oberfläche und erklärtem Prompt
"""
...
""")
dictionary_text = "\n".join(
    [f"{k} = {v}" for k, v in sorted(st.session_state.dictionary.items())]
)
...
if st.session_state.anonymized_text:
    ...
    st.session_state.anonymized_text = st.text_area(
        "Bearbeitbarer Text", st.session_state.anonymized_text, height=300
    )
    if st.button("Text erneut anonymisieren"):
        st.session_state.anonymized_text = anonymize_text(
            st.session_state.original_text, st.session_state.dictionary
        )
...
if st.session_state.anonymized_text:
    ...
    if st.button("KI Analyse starten"):
        ...
        st.markdown(result)
        # Estrae le voci della checklist dall'analisi AI (semplice parsing)
        checklist_lines = []
        if "Empfohlene nächste Schritte für die Kanzlei" in result:
            try:
                ...
                pass
        # Aggiorna checklist nello stato
